...
You will call the function searchSpotify(args) using the parameters you identify from the user's input.

Unlike Jamendo, Spotify uses a more flexible search approach. You can include:
- "track" for specific song names
- "artist" for specific artist names  
- "genre" for music genres
- "tags" for mood, style, or descriptive terms
- "query" for general search terms

You should extract specific artists and track names when mentioned by the user, and use descriptive tags for mood, style, energy level, etc.

Include at least 3-5 relevant search parameters to get good results from Spotify.`,
      ...
      parameters: {
        ...
        properties: {
          track: {
            type: "string",
            description: "Specific track/song name if mentioned by user",
          },
          artist: {
            type: "string", 
            description: "Specific artist name if mentioned by user",
          },
          genre: {
            type: "string",
            description: "Music genre (e.g., rock, pop, jazz, electronic, etc.)",
          },
          tags: {
            type: "array",
            description: "Mood, style, or descriptive tags (e.g., happy, energetic, chill, acoustic)",
            items: { type: "string" },
          },
          query: {
            type: "string",
            description: "General search query if no specific parameters apply",
          },
        },
        additionalProperties: false,
      },
