...
/*
  ...
  We should only use this method for testing or for special cases where cookies are not available (like initial login).
...
export async function authenticateIdToken(request) {
  ...
  try {
    ...
    console.log("🔒 User successfully authenticated with ID token");
    return decodedToken;
  } catch (error) {
    if (error.name === "FirebaseAuthError") {
      return new Response(JSON.stringify({ error: "Unauthorized" }), {
